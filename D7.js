...
console.log(`I film contenuti nell'array sono:" ${length}`);
...
const onlyTitle = (arr) => {
  return arr.map((titoli) => titoli.Title);
};
...
const InThisMillenium = (arr) => {
  return arr.filter((film) => parseInt(film.Year) > 1999);
};
console.log(InThisMillenium(movies));
